...
class ColorApe(FileSystemStorage):
    """
    If a file is not found on disk, return an ape picture.
    """
    def _open(self, name, mode='rb'):
        if not self.exists(name):
            apez = cache.get('apez', False)
            if not apez:
                # Grab an ape from placeape.
                response = urllib2.urlopen('http://placeape.com/%s/%s'
        ...
        return super(ColorApe, self)._open(name, mode)
